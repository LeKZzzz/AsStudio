import scrapy
from ..items import DwmyItem
import re


class DwmySpider(scrapy.Spider):
    name = 'dwmy_spider'  # 爬虫识别名称
    allowed_domains = ['www.cmiyu.com']  # 域名范围
    start_urls = ['http://www.cmiyu.com/dwmy/']  # 爬取的url元组/列表

    def parse(self, response):  # 翻页
        preurl = 'http://www.cmiyu.com/dwmy/my25{}.html'
        for i in range(1, 85):
            url = preurl.format(i)
